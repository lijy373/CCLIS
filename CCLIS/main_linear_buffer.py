...
from util import adjust_learning_rate_linear, warmup_learning_rate_linear, accuracy
...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default='cifar10',
                        choices=['cifar10', 'tiny-imagenet', 'cifar100'], help='dataset')
    ...
    opt.ckpt = os.path.join(opt.ckpt, 'last_weight_{target_task}.pth'.format(target_task=opt.target_task))
    opt.logpt = os.path.join(opt.logpt, 'replay_indices_weight_{target_task}.npy'.format(target_task=opt.target_task))
    return opt
